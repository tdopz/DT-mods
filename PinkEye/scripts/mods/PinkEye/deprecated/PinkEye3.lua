...
local buff_templates = require("scripts/settings/buff/havoc_buff_templates")
-- buff check
local function unit_has_havoc_encroaching_garden(unit)
    ...
    local temp = buff_templates.templates

    local is_t = templates.havoc_encroaching_garden
    if is_t then
        return true
    end
...
-- Main update from latest version
-- mod.update = function(self, dt)
--     if mod:is_in_hub() or mod:is_in_prologue_hub() then
--         return
--     end
